          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-full overflow-hidden bg-white flex-shrink-0">
                <Image
                  src="/Images/barber-pole_17724538.png"
                  alt="Barber Fuze"
                  fill
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                <div className="bg-white border-2 border-cream-300 rounded-xl shadow-lg p-3 sm:p-4 transition hover:shadow-xl">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-cream-200 rounded-full flex items-center justify-center p-2 flex-shrink-0">
                      <Image
                        src="/Icons/user.png"
                        alt="Customers"
                        width={32}
                ...
                <div className="bg-white border-2 border-cream-300 rounded-xl shadow-lg p-3 sm:p-4 transition hover:shadow-xl">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-green-100 rounded-full flex items-center justify-center p-2 flex-shrink-0">
                      <Image
                ...
                <div className="bg-white border-2 border-cream-300 rounded-xl shadow-lg p-3 sm:p-4 transition hover:shadow-xl">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-yellow-100 rounded-full flex items-center justify-center p-2 flex-shrink-0">
                      <Image
                        src="/Icons/clock.png"
                        alt="Pending"
                        width={32}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div className="bg-white border-2 border-cream-300 rounded-xl shadow-lg overflow-hidden">
                  <div className="px-4 py-2.5 sm:py-3 bg-cream-100 border-b-2 border-cream-300">
                    <h3 className="text-sm sm:text-base font-black text-dark-900">Next Appointment</h3>
                  </div>
                  <div className="p-4">
                    {todayStats.nextAppointment ? (
                      <div className="space-y-3 sm:space-y-4">
                        <div className="flex items-center justify-between flex-wrap gap-3">
                          <span className="text-3xl sm:text-4xl font-black text-dark-900">
                            {todayStats.nextAppointment.time}
                          </span>
                          <span className="px-3 sm:px-4 py-1.5 sm:py-2 bg-green-100 text-green-800 text-xs sm:text-sm font-bold rounded-xl border-2 border-green-300 uppercase">
                            {todayStats.nextAppointment.status}
                          </span>
                        </div>
                        <div className="space-y-2 bg-cream-50 rounded-xl p-3 sm:p-4 border-2 border-cream-200">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className="font-bold text-dark-900 text-base sm:text-lg">
                              {todayStats.nextAppointment.name || 'Walk-in Customer'}
                            </p>
                            {todayStats.nextAppointment.queuenumber && (
                              <span className="px-2.5 py-1 rounded-full text-xs font-black bg-dark-900 text-cream-50">
                                Client no.{todayStats.nextAppointment.queuenumber}
                              </span>
                            )}
                          </div>
                          <p className="text-xs sm:text-sm text-dark-700 font-semibold">
                      ...
                      <div className="text-center py-6 sm:py-8">
                        <Image
                          ...
                          width={48}
                          height={48}
                          className="object-contain mx-auto mb-3 sm:mb-4 opacity-40 sm:w-16 sm:h-16"
                        />
                        <p className="text-sm sm:text-base text-dark-600 font-semibold">No appointments remaining</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-white border-2 border-cream-300 rounded-xl shadow-lg overflow-hidden">
                  <div className="px-4 py-2.5 sm:py-3 bg-cream-100 border-b-2 border-cream-300">
                    <h3 className="text-sm sm:text-base font-black text-dark-900">Peak Hours Today</h3>
                  </div>
                  <div className="p-4">
                    <div className="space-y-2.5 sm:space-y-3">
                      {getBusyHours().map(({ hour, count }, index) => (
                        <div key={hour} className="flex items-center justify-between bg-cream-50 rounded-xl p-3 sm:p-4 border-2 border-cream-200">
                          <div className="flex items-center space-x-2 sm:space-x-3">
                            <span className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full text-white text-xs sm:text-sm font-black flex items-center justify-center flex-shrink-0 ${
                              index === 0 ? 'bg-dark-900' : index === 1 ? 'bg-dark-700' : 'bg-dark-500'
                            }`}>
                              {index + 1}
                            </span>
                            <span className="font-black text-dark-900 text-base sm:text-lg">{hour}</span>
                          </div>
                          <span className="text-sm sm:text-base font-bold text-dark-700 whitespace-nowrap">
                            {count} customer{count !== 1 ? 's' : ''}
                          </span>
                        </div>
                      ))}
                      {getBusyHours().length === 0 && (
                        <p className="text-sm sm:text-base text-dark-600 text-center py-4 sm:py-6 font-semibold">No bookings recorded</p>
                      )}
                    </div>
                  </div>
                </div>
